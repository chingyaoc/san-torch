...
function layer:__init(opt)
    ...
    -- LSTM
    local V, D, H = self.vocab_size, self.hidden_size, self.hidden_size
    ...
    self.masking = masking(self.batch_size, self.hidden_size)
...
function masking(batch_size, hidden_size)
    local inputs = {}
    local outputs = {}
...
function layer:updateOutput(input)
  ...
  self.lstm_output = self.net:forward(ques)	-- [batch_size, max_len, dim]

  -- Mask Construction
  self.max_len = ques:size(2)
  local len = torch.LongTensor():resize(seq_len:size()):copy(seq_len)
  local mask_zero = torch.Tensor(self.batch_size,self.max_len):fill(0)
  ...
  --local final_h = torch.CudaTensor(self.batch_size,self.hidden_size)
  --for i = 1, self.batch_size do
  --  final_h[i]:copy(self.lstm_output[i][seq_len[i]])
  --end
...
function layer:updateGradInput(input, gradOutput)
  ...
  --local d_lstm_output = torch.CudaTensor(self.batch_size,self.max_len,self.hidden_size):fill(0)
  --for i = 1, self.batch_size do
  --  d_lstm_output[i][seq_len[i]]:copy(d_ques_feat[i])
  --end
